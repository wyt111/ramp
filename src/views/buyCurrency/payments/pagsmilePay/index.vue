<template>
  ...
  >
    ...
    <!-- 表单-->
    <div class="ArgentinaPay-content" v-else @click="harderSelectclone" key="2">
      <!-- 新增支付方式信息模块 -->
      <PayMethodInfo class="PayMethodInfo" />
      <div
        class="ArgentinaPay-from"
        v-if="payMethodInfo.payWayCode === '11000'"
      >
        <trader-information
          idNumber="identify.pseNumber"
          v-if="!formShowStatus.basicItem"
        />
        <!-- 选择id类型 -->
        <div class="select-inputBox form-item" v-if="formShowStatus.basicItem">
          <div class="id-type">
            <span class="type-title">ID type</span>
            <span class="input-title">{{ formData.pseType }}</span>
          </div>
        </div>
        <!-- ID Number -->
...
export default {
  ...
  data() {
    return {
      ...
      //身份证选择类型数据
      speTypeList: [{ id: 1, name: "CI", idLength: "14" }],
      //数据类型长度
      pseTypeDetails: "14",
      TimeDown: null,
      //失败展示
  ...
  beforeRouteEnter(to, from, next) {
    next((vm) => {
      ...
      if (to.query.isTwice === "1") {
        ...
        sessionStorage.removeItem("pagsmilePayState");
  ...
  mounted() {
    ...
    if (sessionStorage.getItem("pagsmilePayState")) {
      let psePay = JSON.parse(sessionStorage.getItem("pagsmilePayState"));
  ...
  methods: {
    set_disableStatus() {
      ...
      if (this.payMethodInfo.payWayCode === "13000") {
    ...
    backFillTraderInformation() {
      ...
      if (!info.status || sessionStorage.getItem("pagsmilePayState")) return;
      this.formShowStatus.basicItem = false;
      this.formData.pseNumber = info.identify.number;
      this.formData.pseType = info.identify.type;
    },
          ...
          params = {
            ...
            customParam: {
              ...
              identify: {
                ...
                type: this.formData.pseType, // 身份类型 NIT 或 CC (大写)
              },
            },
          };
        }
        ...
        this.$axios
          ...
          .then((res) => {
            if (res && res.data) {
              ...
              sessionStorage.setItem("pagsmilePayState", JSON.stringify(data));
              this.paymentCountStatus();
            }
          })
          .catch((err) => {
